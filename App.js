...
import {
  savedarticlereducer,
  savedarticleintialState,
} from './Redux/savedarticlereducer';
import {savedpostreducer, savedpostintialState} from './Redux/savedpostreducer';
import firestore from '@react-native-firebase/firestore';
import auth from '@react-native-firebase/auth';
import axios from 'axios';
import {
  savedcourseintialState,
  savedcoursereducer,
} from './Redux/savedcoursereducer';
import {
  savedmentorintialState,
  savedmentorreducer,
} from './Redux/savedmentorreducer';
...
const App = () => {
  ...
  const [savedarticlestate, savedarticledispatch] = useReducer(
    savedarticlereducer,
    savedarticleintialState,
  );
  const [savedpoststate, savedpostdispatch] = useReducer(
    savedpostreducer,
    savedpostintialState,
  );
  const [savedcoursestate, savedcoursedispatch] = useReducer(
    savedcoursereducer,
    savedcourseintialState,
  );
  const [savedmentorstate, savedmentordispatch] = useReducer(
    savedmentorreducer,
    savedmentorintialState,
  );
  return (
          ...
          <ChatContext.Provider value={{chatstate, chatdispatch}}>
            <ArticleContext.Provider value={{articlestate, articledispatch}}>
              <SavedArticleContext.Provider
                value={{savedarticlestate, savedarticledispatch}}>
                <NewsContext.Provider value={{newsstate, newsdispatch}}>
                  <CourseContext.Provider value={{coursestate, coursedispatch}}>
                    <SavedPostContext.Provider
                      value={{savedpoststate, savedpostdispatch}}>
                      <SavedCourseContext.Provider
                        value={{savedcoursestate, savedcoursedispatch}}>
                        <SavedMentorContext.Provider
                          value={{savedmentorstate, savedmentordispatch}}>
                          <Routing />
                        </SavedMentorContext.Provider>
                      </SavedCourseContext.Provider>
                    </SavedPostContext.Provider>
