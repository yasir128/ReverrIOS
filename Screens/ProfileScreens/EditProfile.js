...
const EditProfile = () => {
  ...
  const [About, setAbout] = useState(state&&state.about==''?'>/s<':state.about);
  const [Industry, setIndustry] = useState(state&&state.industry==''?'>/s<':state.industry);
  const [loading, setLoading] = useState(false);

  
  const [ex1,setEx1] = useState(state&&state.experience&&state.experience.length>0?state.experience[0]:'>/s<');
  const [ex2,setEx2] = useState(state&&state.experience&&state.experience.length>1?state.experience[1]:'>/s<');
  const [ex3,setEx3] = useState(state&&state.experience&&state.experience.length>2?state.experience[2]:'>/s<');

  const [sk1,setSk1] = useState(state&&state.skills&&state.skills.length>0?state.skills[0]:'>/s<');
  const [sk2,setSk2] = useState(state&&state.skills&&state.skills.length>1?state.skills[1]:'>/s<');
  const [sk3,setSk3] = useState(state&&state.skills&&state.skills.length>2?state.skills[2]:'>/s<');
  ...
  const [ed1,setEd1] = useState(state&&state.education&&state.education.length>0?state.education[0]:'>/s<');
  const [ed2,setEd2] = useState(state&&state.education&&state.education.length>1?state.education[1]:'>/s<');
  const [ed3,setEd3] = useState(state&&state.education&&state.education.length>2?state.education[2]:'>/s<');
  ...
  const saveChanges = async()=>{
    ...
    const Experience =[ex1=='>/s<'?"":ex1,ex2=='>/s<'?"":ex2,ex3=='>/s<'?"":ex3];
    const Skills = [sk1=='>/s<'?"":sk1,sk2=='>/s<'?"":sk2,sk3=='>/s<'?"":sk3]; //sk4,sk5
    const Education =[ed1=='>/s<'?"":ed1,ed2=='>/s<'?"":ed2,ed3=='>/s<'?"":ed3];
